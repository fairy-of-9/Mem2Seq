...
class Mem2Seq(nn.Module):
    ...
    def train_batch(self, input_batches, input_lengths, target_batches, 
        ...
        decoder_hidden = self.encoder(input_batches).unsqueeze(0)  # [1, 8, 128]
        self.decoder.load_memory(input_batches.transpose(0,1))  # story(X+B)에 들어있는 idx로 emb 값을 가져온다.
...
class EncoderMemNN(nn.Module):
    ...
    def forward(self, story):  # [70 8 3]
        story = story.transpose(0,1)  # [8 70 3]  batch(b), dialog_len(m), triple(word, speaker, turn id) 에 해당하는 word2index.
        story_size = story.size() # b * m * 3 
        if self.unk_mask:
            if(self.training):
                ones = np.ones((story_size[0],story_size[1],story_size[2]))  # 8 70 3
                rand_mask = np.random.binomial([np.ones((story_size[0],story_size[1]))],1-self.dropout)[0]  # 8, 70
                ones[:,:,0] = ones[:,:,0] * rand_mask  # 8 70 3
                a = Variable(torch.Tensor(ones))  # 8 70 3
                if USE_CUDA: a = a.cuda()
                story = story*a.long()  # 8 70 3
        u = [self.get_state(story.size(0))]  # 8,128   bsz, embedding_dim  init q^k == query emb (dialog history 인듯.)
        for hop in range(self.max_hops):
            embed_A = self.C[hop](story.contiguous().view(story.size(0), -1).long()) # b * (m * s) * e   8 210 128
            embed_A = embed_A.view(story_size+(embed_A.size(-1),)) # b * m * s * e    8 70 3 128   [8, 70, 3]에 대한 embed 값을 가져옴.


            # TODO
            # weighted sum을 할 수 있지 않을까? alpha = softmax(W * emb).
            # 안해도 어차피 emb 라서 그 모양으로 학습이 될려나. 그럼 그냥 ffnn으로 다시 합치기? 3*e -> e

            m_A = torch.sum(embed_A, 2).squeeze(2) # b * m * e  8 70 128, sum of embs on triple

            u_temp = u[-1].unsqueeze(1).expand_as(m_A)  # 8 70 128  current query vector.
            # u[-1] = [b, emb_dim],
            ...
            prob = prob.unsqueeze(2).expand_as(m_C)  # 8 70 128   8,70의 값을 128개 복제.   prob = word(memory)단위 attention weight
            o_k  = torch.sum(m_C*prob, 1)  # 8 128  현재 hop에서 query와 유사한 애들(attention 사용)을 next hop에서의 embedding에서 가져옴.
            u_k = u[-1] + o_k  # 8 128     #걔들을 query에 더한다? 즉 현재 홉에서 중점적으로 봤던 애들을 다음 홉에서도 더 중요시하게 여기게 한다?
            u.append(u_k)
        return u_k
...
class DecoderMemNN(nn.Module):
    ...
    def ptrMemDecoder(self, enc_query, last_hidden):  # enc_query = previously generated word, last_hidden = previous query
        # enc_query = [b], last_hidden = [1,8,128]
        embed_q = self.C[0](enc_query) # b * e  [8, 128]
        output, hidden = self.gru(embed_q.unsqueeze(0), last_hidden)  # output don't use. return h_t [1,8,128]
        # hidden of shape (num_layers(1) * num_directions(1), batch, hidden_size)
        temp = []
        u = [hidden[0].squeeze()] # list of [8, 128]
        for hop in range(self.max_hops):
            m_A = self.m_story[hop]  # [8, 70, 128] vocab에 대한 memory 값을 가져옴.
            if(len(list(u[-1].size()))==1): u[-1] = u[-1].unsqueeze(0)  # used for bsz = 1.
            u_temp = u[-1].unsqueeze(1).expand_as(m_A) # u[-1].unsqueeze(1) = [8,1,128], u_temp=[8,70,128]   128-d emb을 70개로 늘린다.
            prob_lg = torch.sum(m_A*u_temp, 2)  # [8, 70] vocab에 대한 메모리(128) * ht(128)   == memory 중 current query와 가장 유사한 애.
            prob_   = self.softmax(prob_lg)  # [8, 70]
            m_C = self.m_story[hop+1]  # 8 70 128
            temp.append(prob_)
            prob = prob_.unsqueeze(2).expand_as(m_C)  # 8 70 128
            o_k  = torch.sum(m_C*prob, 1)  # 8, 128
            if (hop==0):
                p_vocab = self.W1(torch.cat((u[0], o_k),1))  # 8,93(vocab size)  u[0] = gru의 맨 처음 output(query vector), o_k = hop attention
            u_k = u[-1] + o_k
            u.append(u_k)
        p_ptr = prob_lg 
